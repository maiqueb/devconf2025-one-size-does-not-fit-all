...
run_cmd kubectl get userdefinednetwork -A -l purpose=devconf-eu-2025-demo
...
run_cmd kubectl get clusteruserdefinednetwork -A -l purpose=devconf-eu-2025-demo
...
sleep 3
run_cmd kubectl get pods -A -l purpose=devconf-eu-2025-demo -owide
...
header "Create two nodeport type services - one in BLUE and GREEN namespaces which expose these statefulsets"
...
run_cmd kubectl get svc -A -l purpose=devconf-eu-2025-demo
...
SVCIP=$(kubectl get nodes ovn-k-helm-worker -o jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}')
...
SVCIP=$(kubectl get nodes ovn-k-helm-worker2 -o jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}')
